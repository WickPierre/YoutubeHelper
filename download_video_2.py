...
def download(url: str):
    ...
    audio_stream = (
        yt.streams.filter(mime_type="audio/mp4").order_by("filesize").desc().first()
    )
    ...
    print("\tFilesize of the video:", round(video_stream.filesize / 1000000), "MB")
    ...
    combine(
        audio_stream.default_filename, video_stream.default_filename, f"{yt.title}.mp4"
    )
